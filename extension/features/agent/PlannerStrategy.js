/**
 * PlannerStrategy - Orchestrates planner execution through phases
 * Phase sequence: Init → StopCheck → Command → SmartSearch → LLMCall → Parsing → PlanUpdate → ToolExecution → Finalization
 */
...
class PlannerStrategy {
  constructor(userText) {
    ...
    this.phases = [
      new PlannerInitializationPhase(),
      new StopCheckPhase(),
  ...
  /**
   * Run the planner strategy with given context
  ...
  async run(context) {
    ...
    while (context.step < context.maxSteps) {
      ...
      for (let i = 1; i < this.phases.length; i++) {
        ...
        } else if (phase instanceof PlanUpdatePhase) {
          // Only run if we have parsed plan
  ...
  handleMaxStepsReached(context) {
    ...
    context.addUiMessage({
      role: 'assistant',
      content: 'Planner stopped: too many steps.'
    });
...
module.exports = { PlannerStrategy };
